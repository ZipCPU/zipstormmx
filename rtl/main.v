...
// check
	...
	// from
	localparam	RESET_ADDRESS = 2097152;
	//
	// The number of valid bits on the bus
	localparam	ZIP_ADDRESS_WIDTH = 21; // Zip-CPU address width
	//
	// ZIP_START_HALTED
	//
...
//
	...
	wire	wd_ireset;	// watchdog.INT.RESET.WIRE
...
`include "builddate.v"
	...
	// Wishbone slave definitions for bus wb(SIO), slave refreshcount
	wire		refreshcount_sel, refreshcount_ack, refreshcount_stall;
	wire	[31:0]	refreshcount_data;
	...
	assign	refreshcount_sel = ((wb_sio_sel)&&(wb_addr[ 2: 0] ==  3'h4));  // 0x000010
	...
	assign	    flashcfg_sel = ((wb_addr[20:16] &  5'h1f) ==  5'h01); // 0x040000
	assign	    sdramdbg_sel = ((wb_addr[20:16] &  5'h1f) ==  5'h02); // 0x080000 - 0x080007
	assign	      wb_dio_sel = ((wb_addr[20:16] &  5'h1f) ==  5'h03); // 0x0c0000 - 0x0c0007
//x2	Was a master bus as well
	assign	     console_sel = ((wb_addr[20:16] &  5'h1f) ==  5'h04); // 0x100000 - 0x10000f
	assign	      wb_sio_sel = ((wb_addr[20:16] &  5'h1f) ==  5'h05); // 0x140000 - 0x14001f
//x2	Was a master bus as well
	assign	       bkram_sel = ((wb_addr[20:16] &  5'h1f) ==  5'h06); // 0x180000 - 0x181fff
	assign	       flash_sel = ((wb_addr[20:16] &  5'h1e) ==  5'h08); // 0x200000 - 0x27ffff
	assign	       sdram_sel = ((wb_addr[20:16] &  5'h18) ==  5'h10); // 0x400000 - 0x5fffff
	//
	...
	assign	      hb_dwb_sel = ((hb_addr[21:21] &  1'h1) ==  1'h0); // 0x000000 - 0x7fffff
//x2	Was a master bus as well
	assign	     zip_dbg_sel = ((hb_addr[21:21] &  1'h1) ==  1'h1); // 0x800000 - 0x800007
	//
	...
	casez( wb_addr[2:0] )
		...
		3'h4: r_wb_sio_data <= refreshcount_data;
	...
	if (wb_stb && ! wb_stall)
		casez(wb_addr[20:16])
			// 007c0000 & 00040000, flashcfg
			5'b000_01: r_wb_bus_select <= 3'd0;
			// 007c0000 & 00080000, sdramdbg
			5'b000_10: r_wb_bus_select <= 3'd1;
			// 007c0000 & 000c0000, wb_dio
			5'b000_11: r_wb_bus_select <= 3'd2;
			// 007c0000 & 00100000, console
			5'b001_00: r_wb_bus_select <= 3'd3;
			// 007c0000 & 00140000, wb_sio
			5'b001_01: r_wb_bus_select <= 3'd4;
			// 007c0000 & 00180000, bkram
			5'b001_10: r_wb_bus_select <= 3'd5;
			// 00780000 & 00200000, flash
			5'b010_0?: r_wb_bus_select <= 3'd6;
			// 00600000 & 00400000, sdram
			5'b10?_??: r_wb_bus_select <= 3'd7;
			default: begin end
		endcase
	...
	casez(r_wb_bus_select)
		...
		3'd2: wb_idata <= wb_dio_data;
		3'd3: wb_idata <= console_data;
		3'd4: wb_idata <= wb_sio_data;
...
`else	// WATCHDOG_ACCESS
	...
	assign	wd_ireset = 1'b0;	// watchdog.INT.RESET.WIRE
...
`endif	// BUSPIC_ACCESS
	...
	assign	refreshcount_data = sdram_refresh_counts;
`ifdef	INCLUDE_ZIPCPU
	...
	wbpriarbiter #(32,21)	bus_arbiter(i_clk,
...
`else
	...
	assign	hb_dwbi_addr  = hb_addr[(21-1):0];
...
`ifdef	BUS_DELAY_NEEDED
	busdelay #(21)	hb_dwbi_delay(i_clk, i_reset,
		hb_dwbi_cyc, hb_dwbi_stb, hb_dwbi_we, hb_dwbi_addr, hb_dwbi_odata, hb_dwbi_sel,
...
`endif	// SDRAM_ACCESS
	...
	assign	buserr_data = { {(32-2-21){1'b0}},
			r_buserr_addr, 2'b00 };
`ifdef	FLASH_ACCESS
...
`endif
	...
	hbconsole genbus(i_clk, hb_rx_stb, hb_rx_data,
			...
			w_console_tx_stb, w_console_tx_data, w_console_busy,
			w_console_rx_stb, w_console_rx_data);
	assign	hb_addr= hb_tmp_addr[(22-1):0];
`else	// WBUBUS_MASTER
