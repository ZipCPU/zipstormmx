...
public:
	...
	virtual	void	decode(DEVBUS::BUSW val) const {
		...
		printf("WB(%s%s%s%s%s",
			...
			(val&0x04000000)?"STL":"   ");
			//
		if ((val&0x64000000)==0x60000000)
			printf("*");
		else
			printf(" ");
		printf(")-SD[%d%d%d%d,B%d]",
			(val&0x02000000)?1:0,
			(val&0x01000000)?1:0,
			(val&0x00800000)?1:0,
			(val&0x00400000)?1:0,
		...
		if (val&0x00200000)
			...
			printf("-> ");
		printf("%s", (val&0x00010000)?"P":" "); // Pending
		printf("@%3x,", (val>>8)&0x0ffc);
		printf(":%d,", (val>>8)&3);
		/*
		printf(",%s%s%s%s%s", 
			(val&0x080)?"R":"-",
	...
	virtual	void define_traces(void) {
		...
		register_trace("o_wb_ack",    1, 27);
		register_trace("o_wb_stall",  1, 26);
		register_trace("o_ram_cs_n",  1, 25);
		register_trace("o_ram_ras_n", 1, 24);
